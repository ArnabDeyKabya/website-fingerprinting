...
def main():
    ...
    website_to_label = {w: i for i, w in enumerate(sorted(set(d['website'] for d in data)))}
    label_to_website = {v: k for k, v in website_to_label.items()}

    traces, labels = [], []
    for d in data:
        if len(d['trace_data']) != INPUT_SIZE:
            continue
        arr = np.array(d['trace_data'])
        norm_trace = ((arr - arr.min()) / (arr.max() - arr.min() + 1e-8)).tolist()
        traces.append(norm_trace)
        labels.append(website_to_label[d['website']])
    ...
    train_dataset = TraceDataset([traces[i] for i in train_idx], [labels[i] for i in train_idx])
    test_dataset = TraceDataset([traces[i] for i in test_idx], [labels[i] for i in test_idx])

    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE)

    models = {
        "Simple": FingerprintClassifier(INPUT_SIZE, HIDDEN_SIZE, len(label_to_website)),
        "Complex": ComplexFingerprintClassifier(INPUT_SIZE, HIDDEN_SIZE, len(label_to_website))
    }

    best_accuracies = {}
    for name, model in models.items():
        print(f"\nTraining {name} model...")
        model_path = os.path.join(MODELS_DIR, f"{name.lower()}_fingerprint_model.pth")
        optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
        criterion = nn.CrossEntropyLoss()
        acc = train(model, train_loader, test_loader, criterion, optimizer, EPOCHS, model_path)
    ...
    print("\nEvaluating best model (Complex)...")
    best_model = ComplexFingerprintClassifier(INPUT_SIZE, HIDDEN_SIZE, len(label_to_website))
    best_model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "complex_fingerprint_model.pth")))
    evaluate(best_model, test_loader, [label_to_website[i] for i in range(len(label_to_website))])
